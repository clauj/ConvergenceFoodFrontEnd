...
const PedidosLojas = () => {
  ...
  useEffect(() => {
    async function getPedidos() {
      ...
      const { data } = await api.get(`pedidos/loja/${params.id}`, config);
      const pedidosReverse = data.pedidos.reverse();
      console.log(pedidosReverse);
      setPedidos(pedidosReverse);
    }
    getPedidos();
  }, [update]);
  ...
  async function aceitarPedido(id_pedido) {
    ...
    const { dataAceita } = await api.put(
      `pedido/${id_pedido}`,
      dataAxios,
      config
    );
  ...
  async function recusarPedido(id_pedido) {
    ...
    const { dataRecusa } = await api.put(
      `pedido/${id_pedido}`,
      dataAxios,
      config
    );
  ...
  return (
    <div className={styles.pedidos}>
      ...
      {pedidos.length === 0 ? (
        <p>Não existem pedidos nesta loja.</p>
      ) : (
        pedidos.map(
          (pedido) =>
                ...
                {pedido.status === "Pendente" ? (
                  ...
                  pedido.status == "Aceito" && (
                    <>
                      ...
                      <button onClick={() => entregaPedido(pedido.id)}>
                        Informar Envio
                      </button>
                    </>
                  )
